...
class load_images_and_labels():  # for training
    def __init__(self, path, batch_size=1, img_size=608, augment=False, randomize=0):
        self.path = path
    ...
    def __next__(self):
        ...
        ib = min((self.count + 1) * self.batch_size, self.nF)  # min is useful for the last batch

        # randomize the image size after every 10 batches
        if self.randomize and self.count % 10 == 0 and not self.count == 0:
            self.height = random.randint(10, 19) * 32
            # if get_current_batch(net)+200 > net['max_batches']:  # TODO might not be necessary
            #     self.height = 608
            print("Resizing to {}".format(self.height))
        ...
        for index, files_index in enumerate(range(ia, ib)):
            ...
            if os.path.isfile(label_path):
                ...
                # class, x coordinate of center, y coordinate of center, box width, box height to
                # class, x coordinate of left, y coordinate of top,
                # x coordinate of right, y coordinate of bot
                labels = labels0.copy()
...
def random_affine(img, targets=None, degrees=(-10, 10), translate=(.1, .1), scale=(.9, 1.1), shear=(-3, 3),
    ...
    if targets is not None:
        if len(targets) > 0:
            ...
            # warp points
            xy = np.ones((n * 4, 3))
            xy[:, :2] = points[:, [0, 1, 2, 3, 0, 3, 2, 1]].reshape(n * 4, 2)  # x1y1, x2y2, x1y2, x2y1, 4 corners of the box
