...
def get_current_batch(net_config):
    return net_config['seen']/(net_config['batch']*net_config['subdivisions'])
...
def build_targets(pred_boxes, pred_conf, pred_cls, target, anchor_wh, nA, nC, nG, requestPrecision):
    """
    returns nGT, nCorrect, tx, ty, tw, th, tconf, tcls  //  t stands for target
    """
    ...
    for b in range(nB):
        ...
        if nTb > 1:
            ...
            # return the unique colums of 2D array (grid_X, grid_Y, anchor_idx)
            # if 2 columns share the same (grid_X, grid_Y, anchor_idx), one with the maximum iou is taken
            ...
            # duplicate (grid_X, grid_Y, anchor_idx) is not used since one anchor can only detect
            # one box in the same cell, discard the duplicate index
            i = iou_order[first_unique]
            # best anchor must share significant commonality (iou) with target
            i = i[iou_anch_best[i] > 0.10]
            if len(i) == 0:
                continue
            # reform the best anchors, gi,  gj and target matrix
